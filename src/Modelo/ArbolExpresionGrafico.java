...
public class ArbolExpresionGrafico extends JPanel {
     ...
     * @param diccionario
     */
    public ArbolExpresionGrafico(Diccionario diccionario) {
    ...
    private void calcularPosiciones() {
        posicionNodos.clear();
        subtreeSizes.clear();
        NodoEntrada root = this.diccionario.getRoot();
        if (root != null) {
     ...
     */
    private Dimension calcularTamañoSubarbol(NodoEntrada n) {
        if (n == null) {
            return new Dimension(0, 0);
        }

        Dimension ld = calcularTamañoSubarbol((NodoEntrada) n.getLi());
        Dimension rd = calcularTamañoSubarbol((NodoEntrada) n.getLd());
     ...
     *
     * @param n: Objeto de tipo NodoEntrada <T> que se utiliza como referencia para
     ...
     */
    private void calcularPosicion(NodoEntrada n, int left, int right, int top) {
        if (n == null) {
            return;
        }

        Dimension ld = (Dimension) subtreeSizes.get((NodoEntrada) n.getLi());
        if (ld == null) {
            ld = empty;
        }

        Dimension rd = (Dimension) subtreeSizes.get((NodoEntrada) n.getLd());
        if (rd == null) {
            rd = empty;
        }
        ...
        calcularPosicion((NodoEntrada) n.getLi(), Integer.MAX_VALUE, center - child2child / 2, top + fm.getHeight() + parent2child);
        calcularPosicion((NodoEntrada) n.getLd(), center + child2child / 2, Integer.MAX_VALUE, top + fm.getHeight() + parent2child);
    }
     ...
     */
    private void dibujarArbol(Graphics2D g, NodoEntrada n, int puntox, int puntoy, int yoffs) {
        if (n == null) {
            return;
        }
        ...
        dibujarArbol(g, (NodoEntrada) n.getLi(), (int) (r.x + r.width / 2), r.y + r.height, yoffs);
        dibujarArbol(g, (NodoEntrada) n.getLd(), (int) (r.x + r.width / 2), r.y + r.height, yoffs);
    ...
    public void paint(Graphics g) {
        ...
        dibujarArbol(g2d, this.diccionario.getRoot(), Integer.MAX_VALUE, Integer.MAX_VALUE,
